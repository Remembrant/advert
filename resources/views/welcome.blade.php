...
<script>
       document.addEventListener("DOMContentLoaded", function () {
        ...
        var markerss = {}; // Store marker instances

        @foreach ($markers as $marker)
            var marker{{ $marker->id }} = L.marker([{{ $marker->lat }}, {{ $marker->lng }}], {
                icon: icons["{{ $marker->color }}"] || icons.red
            }).addTo(map);

            // Click event to open a popup with a form
            marker{{ $marker->id }}.on('click', function () {
                var popupContent = `
                    <b>{{ $marker->name }}</b><br>
                    <form action="/markers/{{ $marker->id }}" method="POST" onsubmit="return submitForm(this, event, {{ $marker->id }})">
                        @csrf
                        @method('PUT')
                        <select name="color">
                            <option value="green" ${'{{ $marker->color }}' === 'green' ? 'selected' : ''}>✅ Online</option>
                            <option value="orange" ${'{{ $marker->color }}' === 'orange' ? 'selected' : ''}>⚠️ Pending</option>
                            <option value="red" ${'{{ $marker->color }}' === 'red' ? 'selected' : ''}>❌ Offline</option>
                        </select>
                        <button type="submit">Update</button>
                    </form>
                `;
                marker{{ $marker->id }}.bindPopup(popupContent).openPopup();
            });

            markers[{{ $marker->id }}] = marker{{ $marker->id }};
        @endforeach
    });
